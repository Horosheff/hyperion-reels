...
| Возможность | Суть |
|-------------|------|
| **Чёткость Q1** | Все даунскейлы — `scale:flags=lanczos` + лёгкий `unsharp` после кадрирования: картинка перестаёт «мылить» |
| **Цвет Q2** | HDR-исходники (iPhone smpte2084/HLG) — `zscale+tonemap=hable` → bt709, больше никакой «серой» картинки; SDR — лёгкий грейд |
| **Encode-полировка** | level 5.1 (4.1 ломал 1080×1920@60), потолок битрейта 12M/24M release, fps-cap 60 для slow-mo источников |
| **Отключаемо** | `VIDEOSHORTS_SHARPEN=0`, `VIDEOSHORTS_COLOR_GRADE=0` |
